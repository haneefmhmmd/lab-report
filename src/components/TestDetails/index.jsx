    ...
    const TestDetail = ({ id, name, value, unit, referenceValue }) => {
        ...
        return (
            <tr className={style.row}>
                ...
                <td>
                    {Array.isArray(referenceValue) ? referenceValue.map((value,index)=>(<p key={uuidv4()}>{value}</p>)): referenceValue}
                    <Button size='md' iconPlacement='only' className={style.tableDeleteBtn} onClick={()=> deleteBtnHandler(id)}>
