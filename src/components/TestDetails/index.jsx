...
export default function TestDetails({ tests, addBtnClickHandler, deleteTestBtnHandler, ...restProps }) {
    ...
    const TestDetail = ({ id, name, value, unit, normalValue }) => {
        ...
        <section {...restProps}>
            ...
            <article className={style.tableWrapper}>
                <table className={style.table}>
                    ...
                    <tbody>
                        {
                            tests.length == 0 ? <NoTestAvailable /> : tests.map((test) => (<TestDetail id={test.id} name={test.name} value={test.value} unit={test.unit} normalValue={test.referenceValue} key={`selected-test-${test.id}`} />))
                        }
                    </tbody>
                </table>
            </article>
